...
class SimpleType(Type, CallableValue):
    ...
    def __read__(self, s):
        ...
        if s.dataLen() < self._typeSize:
            ...
            raise ValueError("Stream is too small to read expected SimpleType")
            ...
            try:
                ...
                if not self._readLen is None and readLen > self._readLen.value:
                    ...
                    if not self.__dict__[name]._optional:
                        raise ValueError("Impossible to read type %s : read length is too small"%(self.__class__))
                        
                        # raise InvalidSize("Impossible to read type %s : read length is too small"%(self.__class__))
                
            except Exception as e:
                logging.error("Error during read %s::%s"%(self.__class__, name))
    ...
    def __write__(self, s):
        ...
        for name in self._typeName:
            try:
                ...
                logging.error("Error during write %s::%s"%(self.__class__, name))
                raise e
