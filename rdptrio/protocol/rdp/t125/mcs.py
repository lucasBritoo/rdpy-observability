...
# from rdptrio.core.type import UInt16Le
...
import rdptrio.protocol.rdp.t125.gcc as gcc
...
class MCSLayer():
    ...
    def __init__(self, receiveOpcode, sendOpcode, virtualChannels = []):
        """
        @param presentation: {Layer} presentation layer
        ...
        #default user Id
        self._userId = 1 + config.get_value("channelId", "MCS_USERCHANNEL_BASE")
        #list of channel use in this layer and connection state
        self._channels = {config.get_value("channelId", "MCS_GLOBAL_CHANNEL"): "verificar presentation"}
        #virtual channels
...
class Client(MCSLayer):
    ...
    """
    def __init__(self, virtualChannels = []):
        """
        @param presentation: {Layer} presentation layer
        @param virtualChannels: {Array(Layer)} list additional channels like rdpsnd... [tuple(mcs.ChannelDef, layer)]
        """
        logger.debug('[MCSClient]')
        MCSLayer.__init__(self, config.get_value("domainMCSPdu", "SEND_DATA_INDICATION"), config.get_value("domainMCSPdu", "SEND_DATA_REQUEST"), virtualChannels)
