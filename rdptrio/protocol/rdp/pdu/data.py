...
class PDU(CompositeType):
    ...
    def __init__(self, userId = 0, pduMessage = None):
        ...
        def PDUMessageFactory():
            ...
            logging.debug("unknown PDU type : %s"%hex(self.shareControlHeader.pduType.value))
            #read entire packet
        ...
        if pduMessage is None:
            ...
            raise ValueError("Try to send an invalid PDU")
...
class DataPDU(CompositeType):
    ...
    def __init__(self, pduData = None, shareId = 0, readLen = None):
        ...
        def PDUDataFactory():
            ...
            logging.debug("unknown PDU data type : %s"%hex(self.shareDataHeader.pduType2.value))
        ...
        if pduData is None:
            ...
            raise ValueError("Try to send an invalid data PDU")
...
class UpdateDataPDU(CompositeType):
    ...
    def __init__(self, updateData = None, readLen = None):
        ...
        def UpdateDataFactory():
            ...
            logging.debug("unknown PDU update data type : %s"%hex(self.updateType.value))
        ...
        if updateData is None:
            ...
            raise ValueError("Try to send an invalid data update PDU")
...
class FastPathUpdatePDU(CompositeType):
    ...
    def __init__(self, updateData = None):
        ...
        def UpdateDataFactory():
            ...
            logging.debug("unknown Fast Path PDU update data type : %s"%hex(self.updateHeader.value & 0xf))
        ...
        if updateData is None:
            ...
            raise ValueError("Try to send an invalid fast path data update PDU")
...
class SlowPathInputEvent(CompositeType):
    ...
    def __init__(self, messageData = None):
        ...
        def SlowPathInputDataFactory():
            for c in [PointerEvent, PointerExEvent, ScancodeKeyEvent, UnicodeKeyEvent, SynchronizeEvent]:
                if self.messageType.value == c._INPUT_MESSAGE_TYPE_:
                    return c()
            raise ValueError("unknown slow path input : %s"%hex(self.messageType.value))
        ...
        if messageData is None:
            ...
            raise ValueError("try to send an invalid Slow Path Input Event")
