...
from rdptrio.core.type import CallableValue, UInt32Le, UInt16Le, String, sizeof
from rdptrio.protocol.rdp.t125 import gcc
from rdptrio.core.config import Config
from rdptrio.protocol.rdp.t125 import gcc
import logging

#import hashlib
#import rdptrio.protocol.rdp.lic as lic
#import rdptrio.protocol.rdp.tpkt as tpkt
#from rdptrio.protocol.rdp.t125 import mcs
#from rdptrio.core.type import CompositeType, CallableValue, UInt32Le, UInt16Le, String, sizeof#, Stream, UInt8
# from rdptrio.core.layer import IStreamSender
# from rdpy.core.error import InvalidExpectedDataException
# from rdpy.core import log
#from rdptrio.security import rc4
...
class SecLayer():#IStreamSender, tpkt.IFastPathListener, tpkt.IFastPathSender, mcs.IGCCConfig):
    """
    ...
    def __init__(self):
        ...
        """
        logger.debug('[SecLayer]')
        # LayerAutomata.__init__(self, presentation)
        #thios layer is like a fastpath proxy
        self._fastPathTransport = None
        self._fastPathPresentation = None
        
        rdp_version = config.get_value("versions", "RDP_VERSION_5_PLUS")

        #credentials
        self._info = RDPInfo(extendedInfoConditional = lambda:(gcc.serverSettings().serverCore.rdpVersion._value == rdp_version) )
    ...
    #         logger.debug("update decrypt key")
    ...
    #         logger.debug("update encrypt key")
...
class Client(SecLayer):
    ...
    """
    def __init__(self):
        logger.debug('[SecClient]')
        SecLayer.__init__(self)
    ...
    #         logger.warning("cannot verify server identity")
    ...
    #         raise ValueError("waiting license packet")
...
class RDPInfo():#CompositeType):
    ...
    def __init__(self, extendedInfoConditional):
        ...
        #support flag
        info_mouse = config.get_value("infoFlag", "INFO_MOUSE")
        info_unicode = config.get_value("infoFlag", "INFO_UNICODE")
        info_logonNotify = config.get_value("infoFlag", "INFO_LOGONNOTIFY")
        info_logonErrors = config.get_value("infoFlag", "INFO_LOGONERRORS")
        info_disableTrl = config.get_value("infoFlag", "INFO_DISABLECTRLALTDEL")
        info_enableWindow = config.get_value("infoFlag", "INFO_ENABLEWINDOWSKEY")
          
        self.flag = UInt32Le(info_mouse | info_unicode | info_logonNotify | info_logonErrors | info_disableTrl | info_enableWindow)
...
class RDPExtendedInfo():#CompositeType):
    ...
    def __init__(self, conditional):
        ...
        self.clientAddressFamily = UInt16Le(config.get_value("afInet", "AF_INET"))
...
#             raise ValueError("waiting client random")
...
#             raise ValueError("Waiting info packet")
